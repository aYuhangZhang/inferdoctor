...
def _detect_template_type(root: Path) -> str:
    if (root / "data" / "faq.md").exists():
        return "customer-service"
    if (root / "data" / "menu.yaml").exists():
        return "restaurant-ordering"
    if (root / "query.py").exists() and (root / "docs").exists():
        return "local-doc-qa"
    return "unknown"
...
def validate_template_project(path: str) -> TemplateValidationReport:
    ...
    if template_type == "customer-service":
        ...
        items.append(TemplateValidationItem("FAQ data", "PASS" if exists else "FAIL", "Found data/faq.md." if exists else "Missing data/faq.md.", "Add FAQ content under data/faq.md."))
    if template_type == "restaurant-ordering":
        exists = (root / "data" / "menu.yaml").exists() and (root / "data" / "policies.md").exists()
        items.append(TemplateValidationItem("restaurant data", "PASS" if exists else "FAIL", "Found menu and policy data." if exists else "Missing menu or policy data.", "Add data/menu.yaml and data/policies.md."))
...
def render_template_validation(report: TemplateValidationReport) -> str:
    ...
    lines.extend([
        ...
        "  python app.py" if report.template_type in {"customer-service", "restaurant-ordering"} else "  python ingest.py && python query.py",
        "",
