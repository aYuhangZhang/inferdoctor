...
def render_setup_plan(plan: SetupPlan) -> str:
    lines = [
        ...
        "  Why: {0}".format(plan.reason),
        "",
        "Next commands:",
        "  {0}".format(plan.diagnosis_command),
        "  inferdoctor template show {0}".format(plan.template),
        "  {0}".format(plan.template_command),
        "",
        "InferDoctor init is advisory. It does not install runtimes, download models, or modify system settings.",
    ]
    return "\n".join(lines)
