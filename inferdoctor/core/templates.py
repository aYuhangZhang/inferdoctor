...
def render_template_create_summary(name: str, output_dir: str, written: list[str]) -> str:
    ...
    lines.extend([
        ...
        "  python app.py" if name in {"customer-service", "restaurant-ordering"} else "  python ingest.py && python query.py",
        "",
        "Diagnose the endpoint first if the demo cannot connect:",
        "  inferdoctor check vllm --endpoint http://127.0.0.1:8000/v1",
    ])
    return "\n".join(lines)
